...
public class Chunk : MonoBehaviour
{
    ...
    public void GenerateTerrain(int seed, int chunkCount)
    {
        ...
        for (int x = 0; x < ChunkSize; x++)
        {
            ...
            {
                float perlinValue = Mathf.PerlinNoise((transform.position.x + x + seed) * perlinScale,
                    (transform.position.z + z + seed) * perlinScale);
        ...
        for (int x = 0; x < ChunkSize; x++)
        {
            for (int y = 0; y < ChunkSize; y++)
            {
                for (int z = 0; z < ChunkSize; z++)
                {
                    ...
                    if (currentBlock.getType().Equals(BlockType.Air)) continue; // Skip air blocks
    ...
    {
        // Check neighboring blocks for air
        if (x > 0 && blocks[x - 1, y, z].getType() == BlockType.Air) return true;
        if (x < ChunkSize - 1 && blocks[x + 1, y, z].getType() == BlockType.Air) return true;
        if (y > 0 && blocks[x, y - 1, z].getType() == BlockType.Air) return true;
        if (y < ChunkSize - 1 && blocks[x, y + 1, z].getType() == BlockType.Air) return true;
        if (z > 0 && blocks[x, y, z - 1].getType() == BlockType.Air) return true;
    ...
    private Chunk GetNeighborChunk(int x, int z)
    {
        ...
        Collider[] colliders = Physics.OverlapBox(neighborChunkPos + new Vector3(ChunkSize / 2f, 0, ChunkSize / 2f),
            new Vector3(ChunkSize / 2f, ChunkSize / 2f, ChunkSize / 2f));
    ...
    private void GenerateTrees(int seed)
    {
        ...
        for (int x = 0; x < ChunkSize; x++)
        {
            for (int z = 0; z < ChunkSize; z++)
            {
                ...
                if (height >= 0 &&
                    random.NextDouble() < treeSpawnProbability) // Adjust the probability (5%) for tree generation
                {
    ...
    private int CalculateSurfaceHeight(int x, int z)
    {
        ...
        {
            if (blocks[x, y, z] != null && blocks[x, y, z].getType() != BlockType.Air)
            {
                return y; // Found a non-empty block (surface)
            }
        }

        return -1; // No solid block found (shouldn't happen in a
    }
} 